...
Receives a job page URL, selects a scraper based on the URL hostname and path, and returns a normalized `ScrapedJob`. LinkedIn `/jobs/view/...` URLs are the first supported job page source.

Example request:

```bash
curl -X POST http://localhost:3000/scrape/job-page \
  -H "Content-Type: application/json" \
  -d '{"url":"https://de.linkedin.com/jobs/view/web-developer-m-w-d-at-yootheme-4302641323?position=1&pageNum=0&refId=WdRxYQpg4kqDzW5N7YPKGg%3D%3D&trackingId=IdULDrVZpNO9H345reQ6xQ%3D%3D"}'
```
...
```json
{
  ...
  "sourceJobId": "4302641323",
  "sourceUrl": "https://de.linkedin.com/jobs/view/web-developer-m-w-d-at-yootheme-4302641323/",
  ...
  "tags": ["Full-time"],
  "duplicateKey": "linkedin:4302641323"
}
```

The route accepts any absolute URL in the request body, but it only invokes a scraper when the hostname and path match a registered job page scraper. Unsupported job page URLs return a `422` response.
