...
describe("buildLinkedInJobSearchUrl", () => {
  it("defaults pageNum to 0 when omitted", () => {
    const url = new URL(
  ...
  it("sets the given pageNum explicitly", () => {
    const url = new URL(
      buildLinkedInJobSearchUrl("TypeScript", "Berlin", 25, "86400", 3),
    );
    expect(url.searchParams.get("pageNum")).toBe("3");
  });
