...
const segmentCoverLetterWithLlmFallback = jest.fn<(normalizedCoverLetterText: string) => Promise<CoverLetterTextSegments>>();

jest.unstable_mockModule("./coverLetterSegmentationFallback.js", () => ({
    segmentCoverLetterWithLlmFallback,
}));

const { segmentCoverLetter, segmentCoverLetterHeuristically } = await import("./coverLetterSegmentation.js");
...
describe("cover letter segmentation", () => {
    ...
    it("segments a German cover letter with subject, salutation, body, and greetings", () => {
        const result = segmentCoverLetterHeuristically(`Betreff: Bewerbung als Fachinformatiker
...
Ole`);
        ...
        expect(result.segments).toEqual({
            ...
            mainBody: "In meinen Projekten arbeite ich mit TypeScript, Node.js und modernen Entwicklungsprozessen.",
...
Ole`);
    ...
    it("uses the LLM fallback when heuristic confidence is low", async () => {
        const input = "This is an unusual one paragraph application without conventional markers.";
