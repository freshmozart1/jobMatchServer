...
export function isCoverLetterTextSegments(
  value: unknown,
): value is CoverLetterTextSegments {
  return (
    typeof value === 'object' &&
    value !== null &&
    'subject' in value &&
...
export async function segmentCoverLetterWithLlmFallback(
  normalizedCoverLetterText: string,
): Promise<CoverLetterTextSegments> {
  ...
  if (
    !validateSourcePreservingSegments(
      normalizedCoverLetterText,
      normalizedSegments,
    )
  ) {
    throw new Error(
