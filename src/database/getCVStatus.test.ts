...
import { mockMongoDbModule, connect, close } from "../testMockModules/mongodb.test.js";
...
describe("getCVStatus", () => {
    ...
    it("returns 404 when the job is not found", async () => {
        ...
        expect(json).toHaveBeenCalledWith({ error: "Job not found", message: "Error checking CV status" });
    ...
    it("returns 404 when the CV is not found", async () => {
        ...
        expect(status).toHaveBeenCalledWith(404);
        expect(json).toHaveBeenCalledWith({ error: "CV not found", message: "Error checking CV status" });
        expect(close).toHaveBeenCalledTimes(1);
    });
    ...
    it("returns 500 on a database error", async () => {
        ...
        expect(json).toHaveBeenCalledWith({ error: "DB connection failed", message: "Error checking CV status" });
        expect(close).toHaveBeenCalledTimes(1);
    });
});
