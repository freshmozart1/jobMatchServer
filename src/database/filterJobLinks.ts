...
function isValidLinkedInJobLinksByKeyword(
  body: unknown,
): body is LinkedInJobLinksByKeyword {
  return (
    typeof body === 'object' &&
    body !== null &&
    !Array.isArray(body) &&
...
export default async function filterJobLinks(
  ...
  const requestBodyMustBeObjectError = new Error(
    'Request body must be an object mapping keywords to URL arrays',
  );

  if (!isValidLinkedInJobLinksByKeyword(request.body)) {
    createErrorMessage(
      response,
      requestBodyMustBeObjectError,
      requestBodyMustBeObjectError.message,
      400,
    );
    return;
  }
  ...
  try {
    ...
    const storedJobs = await jobsCollection
      .find(
        { sourceUrl: { $in: uniqueUrls } },
        { projection: { sourceUrl: 1, _id: 0 } },
      )
      .toArray();
    const storedSourceUrls = new Set(
      storedJobs.map(({ sourceUrl }) => sourceUrl),
    );
    ...
    createErrorMessage(
      response,
      error,
      'An error occurred while filtering job links',
    );
  } finally {
    await client.close();
  }
