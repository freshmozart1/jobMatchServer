...
type MockMergedDoc = {
  ...
  embedJpg: (bytes: Buffer) => Promise<MockPdfImage>;
  embedPng: (bytes: Buffer) => Promise<MockPdfImage>;
};
...
const mockEmbedJpg = jest.fn<(bytes: Buffer) => Promise<MockPdfImage>>();
const mockEmbedPng = jest.fn<(bytes: Buffer) => Promise<MockPdfImage>>();
...
describe("getApplication", () => {
  ...
  it("embeds PNG certificate using pdf-lib without a browser page", async () => {
    ...
    // Browser is used only for the cover letter — image certs use pdf-lib.
    expect(mockLaunch).toHaveBeenCalledTimes(1);
    expect(mockBrowserClose).toHaveBeenCalledTimes(1);
    expect(mockNewPage).toHaveBeenCalledTimes(1);
    expect(mockPageClose).toHaveBeenCalledTimes(1);
    expect(mockPdf).toHaveBeenCalledTimes(1);
    // Cover letter + CV are loaded as PDFDocuments; the PNG is embedded natively.
  ...
  it("skips certificates with unrecognised MIME types without failing", async () => {
    ...
    // Unrecognised MIME type: only the cover letter uses a browser page.
    expect(mockNewPage).toHaveBeenCalledTimes(1);
    expect(mockPageClose).toHaveBeenCalledTimes(1);
