...
type FindResultMock = {
    toArray: ReturnType<typeof jest.fn<() => Promise<CoverLetterDocument[]>>>;
};
...
const toArray = jest.fn<() => Promise<CoverLetterDocument[]>>();
...
describe("getTopXSimilarCoverLetters", () => {
    ...
    it("returns the top x cover letters sorted by cosine similarity", async () => {
        const firstEmbedding = [0.1, 0.2, 0.3] satisfies TextEmbedding;
        const secondEmbedding = [0.4, 0.5, 0.6] satisfies TextEmbedding;
        const thirdEmbedding = [0.7, 0.8, 0.9] satisfies TextEmbedding;
