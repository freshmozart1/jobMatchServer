...
describe("getCV", () => {
    ...
    it("returns 404 when the job is not found", async () => {
        ...
        expect(json).toHaveBeenCalledWith({ error: "Job not found", message: "Error retrieving CV" });
    ...
    it("returns 404 when the CV is not found", async () => {
        ...
        expect(status).toHaveBeenCalledWith(404);
        expect(json).toHaveBeenCalledWith({ error: "CV not found", message: "Error retrieving CV" });
        expect(close).toHaveBeenCalledTimes(1);
    });
});
