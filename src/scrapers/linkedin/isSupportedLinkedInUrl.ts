...
export default function isSupportedLinkedInUrl(url: string, variant: LinkedInUrlVariant): boolean {
    try {
        const parsedUrl = new URL(url);

        return (isLinkedInHost(parsedUrl.hostname) && parsedUrl.protocol === "https:") && (
            (variant === "jobPage" && getLinkedInJobPathSegment(parsedUrl) !== null) ||
