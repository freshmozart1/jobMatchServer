...
async function getLikedEmbeddings(
...
async function computeAverageLikedJobSimilarity(
  ...
  return calculateCosineSimilarity(
    sum.map((v) => v / likedEmbeddings.length),
    embedding,
  );
}
...
export function parseCompanyAddress(
  ...
  const streetMatch = p1.match(/^(.+?)\s+(\d\S*)$/);
  ...
  const [city, statePlusPostal, countryCode] = parts;
  if (!city || !statePlusPostal || !countryCode) return null;
  const postalCode = statePlusPostal.split(/\s+/).pop();
...
async function extractLinkedInJobPage(
  page: Page,
