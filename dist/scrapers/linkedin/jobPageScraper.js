...
export async function scrapeLinkedInJobPage(request, response) {
    ...
    try {
        ...
        if (!canonicalUrl)
            throw couldntNormalizeUrlError;
        ...
        let similarity;
        const likedJobsEmbeddings = (await getCollection(client, 'jobs').find({ like: true }).toArray()).map(j => j.embedding);
        ...
        createErrorMessage(response, error, "Failed to scrape job page.", error instanceof Error && error.message === bodyHasNoUrlError.message
            ? 400
...
export function getUrlFromBody(body) {
...
async function extractLinkedInJobPage(page) {
    ...
    return page.evaluate(() => {
        ...
        function normalizeRenderedDescription(value) {
        ...
        function wrapFormattedDescriptionText(marker, value) {
        ...
        function renderDescriptionNodes(nodes) {
            return nodes.map(renderDescriptionNode).join("");
        }
        function getFirstDescription(selectors) {
            for (const selector of selectors) {
                const element = document.querySelector(selector);
        ...
        function getJobPostingJsonLd() {
            ...
            function collect(value) {
                ...
                const isJobPosting = typeValues.some((typeItem) => typeof typeItem === "string" && typeItem.toLowerCase() === "jobposting");
                if (isJobPosting) {
                    candidates.push(value);
                }
            }
            for (const scriptElement of Array.from(document.querySelectorAll('script[type="application/ld+json"]'))) {
                try {
        ...
        function getHiringOrganizationName(jobPosting) {
        ...
        function getTags(jobPosting) {
            const values = new Set(getAllTexts(tagSelectors));
            for (const key of ["employmentType", "industry", "occupationalCategory"]) {
...
function normalizeLinkedInJobPageUrl(jobUrl) {
    try {
...
function isModalOrLegalText(value) {
    const normalizedValue = value.toLowerCase();
    return (normalizedValue.includes("einloggen") && normalizedValue.includes("linkedin") && normalizedValue.includes("mitglied werden")) || (normalizedValue.includes("sign in") && normalizedValue.includes("linkedin") && normalizedValue.includes("join now"));
}
//# sourceMappingURL=jobPageScraper.js.map