...
const mockCreateJobEmbedding = jest.fn();
...
function createPageMock({ url = validLinkedInJobUrl, title = "Software Engineer at Acme Corp | LinkedIn", extractedPage = defaultExtractedPage, } = {}) {
    return {
        click: jest.fn().mockResolvedValue(undefined),
        evaluate: jest.fn().mockResolvedValue(extractedPage),
...
describe("scrapeLinkedInJobPage", () => {
    ...
    it("responds 200 with the scraped job data on success", async () => {
        ...
        const jobData = json.mock.calls[0]?.[0];
    ...
    it("includes a valid ISO scrapedAt timestamp", async () => {
        ...
        expect(jobData["scrapedAt"] >= before).toBe(true);
        expect(jobData["scrapedAt"] <= after).toBe(true);
    });
...
//# sourceMappingURL=jobPageScraper.test.js.map