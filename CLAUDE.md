...
- **Two-layer duplicate skipping in `scrapeJob.ts`** (#119): before each `runScrape` call, `getStoredSourceJobIds` pre-fetches every stored `sourceJobId` into a `Set` once per request and builds a synchronous `shouldScrapeJob` predicate from it, passed to `linkedin-job-scraper` (`v0.12.0`) via `scraperOptions`. The scraper checks this per job card _before_ clicking into it, skipping the click/detail-pane-read/company-lookup entirely for jobs already known (emitted as a `status: 'skipped'` `job:done` event, handled by a dedicated switch case in `handleProgressEvent`) instead of paying the full scrape cost and discarding the result afterward. This pre-click filter is a best-effort optimization only — the pre-fetch runs after `res.writeHead` inside the client-disconnect `try`/`finally` so a failure doesn't delay the stream's first byte or leak the `MongoClient`, and a failed fetch just falls back to an empty `Set` (scrape everything, pre-#119 behavior) rather than aborting the request. The existing post-scrape `duplicateKey` check above remains the correctness safety net: it's the only check that runs for jobs with no `sourceJobId` (matched by normalized URL instead) and for any job the pre-click filter missed because the pre-fetch itself failed.
- **LinkedIn detail-pane identity gate and overlay diagnostics** (#126): `linkedin-job-scraper` v0.12.0 only returns a successful result after the detail title-link's parsed posting ID exactly matches the clicked card's `sourceJobId`. A persistent mismatch becomes `status: 'failed'` with `failureReason: 'detail-pane-identity-unverified'` and remains eligible for the package's single deferred retry; `handleProgressEvent` logs that failure but never sends it through dedupe, embedding, matching, or SSE. The handler exhaustively consumes the package's progress union. `overlay:undismissed` is logged with the package's length-capped `describeOverlayDiagnostics` rendering, while `jobs:loading`, `jobs:found`, and `job:start` remain intentionally unforwarded until #122. Do not serialize whole progress events or job diagnostics: v0.12.0 can attach diagnostic records containing optional DOM snapshots.
- **`POST /scrape/linkedin`'s SSE frame contract** (#124): the stream opens with `: ping\n\n`, an SSE _comment_ whose only job is flushing the headers so the client's `fetch()` resolves and the stream opens. It fires once and is not a keepalive (a periodic one is #122); the bare `ping\n\n` it replaced flushed identically but, having no colon, parsed as an unrecognized field name and was ignored by every reader. A run-level failure writes `data: {"error":"Scrape failed","reason":"<message>"}`, with `reason` built by `describeFailureReason` — **only a message ever goes on the wire, never the error object**: an `Error`'s own fields are non-enumerable, so `JSON.stringify` renders the error itself as `{}`, while spreading it would leak internal state (`ScrapeAbortedError` carries the entire partial results array). Its three fallbacks are each load-bearing — `message || name` because `new Error().message` is `''` and a falsy reason is as useless as `{}`; an error-like object's own `message` because a `{ message, code }` rejection or a cross-realm `Error` fails `instanceof` and `String()` would flatten it to `"[object Object]"`; and `String()` inside a `try`/`catch` because it throws on a null-prototype rejection, and that throw would escape past the handler's `res.end()` and strand the stream open.
- **Cover letter pipeline** spans two endpoints and delegates segmentation, embedding, ranking, and generation entirely to the `cover-letter-generator` package — jobMatchServer's own code is limited to MongoDB access, the Express layer, and `StoredCoverLetter` <-> package-type adapters. `POST /cover-letters/upload/text` (`src/database/uploadCoverLetterAsText.ts`) stores letters _segmented_ (`subject`/`salutation`/`introduction`/`mainBody`/`conclusion`/`greetings`, see `CoverLetterSegmentName` in `src/types.ts`), each with its own embedding: `segmentCoverLetter` (heuristic first, LLM fallback) and `embedCoverLetterSegments` do the actual work, with local `toStoredCoverLetter`/`toStoredCoverLetterSegment` adapters converting the package's `CoverLetter` type to this repo's stored shape. `POST /cover-letters/create/text` (`src/coverLetters/generateCoverLettersAsText.ts`) fetches every stored cover letter, converts it to the package's shape via `src/coverLetters/coverLetterAdapters.ts` (`toGeneratorCoverLetter`, `getGeneratorCoverLetterTextSegments`), then hands ranking and generation to the package in one round trip: `embedJob` embeds the target job, `getTopXSimilarCoverLetters` ranks stored letters against it by a fixed per-segment weighting (`mainBody` dominant) rather than a single whole-letter embedding, and `generateCoverLetter` writes the new letter from the top `x` matches (default 3). There is no `getTopXSimilarCoverLetters.ts` in this repo anymore — ranking is no longer a separate endpoint or local module, and the weighting itself lives in the package, not here.
