...
- **Job scoring pipeline** (`src/scrapers/linkedin/scrapeJob.ts` → `src/embeddings/jobEmbedding.ts` → `src/scrapers/linkedin/linkedInJobSimilarity.ts`): a scraped job is normalized, embedded, then scored against the average embedding of previously liked jobs (`like: true` docs) minus disliked ones, with liked/disliked embedding sets cached in-memory for 30s (`CACHE_TTL_MS` in `linkedInJobSimilarity.ts`) to avoid refetching on every job in a batch. `duplicateKey` is the dedupe key used to upsert on `POST /jobs/create`; `scrapeJob.ts` does not yet compute a real one or filter already-stored jobs during scraping (tracked separately). `src/scrapers/linkedin/playwright/` is the earlier local Playwright-based scraper, no longer wired to any route — dead code pending removal.
