...
- **Cover letter pipeline** (`src/coverLetters/`): letters are stored _segmented_ (`subject`/`salutation`/`introduction`/`mainBody`/`conclusion`/`greetings`, see `CoverLetterSegmentName` in `src/types.ts`), each with its own embedding. Segmentation itself (heuristic first, LLM fallback) is delegated to `segmentCoverLetter` from the `cover-letter-generator` package; `coverLetterAdapters.ts` converts between its `CoverLetterSegments`/`CoverLetter` types and this repo's stored/embedded representation (`toGeneratorCoverLetter`, `getGeneratorCoverLetterTextSegments`). `POST /cover-letters/create/text` (`generateCoverLettersAsText.ts`) fetches every stored cover letter, then hands ranking and generation to the package in one round trip: `embedJob` embeds the target job, `getTopXSimilarCoverLetters` ranks stored letters against it by a fixed per-segment weighting (`mainBody` dominant) rather than a single whole-letter embedding, and `generateCoverLetter` writes the new letter from the top `x` matches (default 3). Ranking is no longer a separate endpoint or local module — there is no `getTopXSimilarCoverLetters.ts` in this repo anymore, and the weighting itself lives in the package, not here.
